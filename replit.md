...
- **GigScore System (Oct 31, 2025):** Comprehensive worker performance scoring algorithm calculating 0-100 point scores based on weighted factors: review quality (40% - average star ratings), completed jobs (25% - logarithmic scale), response time (15% - faster = higher score), cancellations (10% - lower cancellations = higher score), repeat clients (10% - client loyalty indicator). Features tier badges (Rising 0-39, Pro 40-69, Elite 70-89, Legend 90-100), API endpoints for calculation and persistence, and prominent UI display on profile pages with clickable modal showing detailed component breakdown.
- **GigScore Interactive Modal (Oct 31, 2025):** Clickable GigScore section on profile pages opens shadcn Dialog with detailed breakdown showing total score, tier badge, all 5 components with progress bars and weights, and tier reference guide. Provides transparency into scoring algorithm.
- **How It Works Page (Oct 31, 2025):** Comprehensive onboarding guide at `/how-it-works` explaining platform overview, client workflow (Browse/Choose/Book/Review), giger workflow (Profile/Price/Accept/Build), pricing models, GigScore system, and safety features. Accessible from both desktop and mobile navigation menus.
- **Dual-View Dashboard with Tasker Branding (Oct 31, 2025):** Redesigned dashboard with toggle switch separating Giger and Tasker experiences. Giger Dashboard displays Total Earnings, Active Gigs, Completed Gigs, and Total Taskers served with service booking management. Tasker Dashboard shows Total Spent, Active Bookings, Posted Tasks, and Services Used with request management. Introduced "Tasker" as the branded name for customers in the Gigifypro ecosystem. Mock data included for demonstration of both views.
